...
const TRACK_CONTINUATION = {
  python: ["JS Essentials", "Introduction to Databases", "Data Analytics Foundations"],
  sql: ["Python for DSML", "Data Analytics Foundations", "JS Essentials"],
  frontend: ["Programming Foundations", "Introduction to Databases", "Node JS", "MongoDB"],
  backend: ["Build Your Own Static Website", "Build Your Own Responsive Website", "Introduction to React JS"],
...
export const generatePath = async (req, res) => {
  try {
    ...
    async function appendCourse(courseName) {
      const items = await Curriculum.find({ courseName }).sort({ sequenceNumber: 1 });
      let addedMins = 0;
    ...
    let suggestion = null;
    const lastModuleCourses = modules.length > 0
      ? modules[modules.length - 1].coursesInWeek || []
      : [];
    const lastCourseInPlan = lastModuleCourses[lastModuleCourses.length - 1] || "";
